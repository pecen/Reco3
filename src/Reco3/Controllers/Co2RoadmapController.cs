						...
						for (int i = 0; i < Request.Files.Count; i++)
						{
							...
							using (StreamReader reader = new StreamReader(fs, Encoding.UTF8))
							{
								...
								if (false == fleet.InitializeFromXML(map.XML, map.XMLSchemaFilename))
								{
									...
									return Json(new { success = false, message = $"Failed to save roadmap. Internal error while validating the baseline. The following VINs were failing: {vins}" }, JsonRequestBehavior.AllowGet); // {map.XMLSchemaFilename}." }, JsonRequestBehavior.AllowGet);
								}
